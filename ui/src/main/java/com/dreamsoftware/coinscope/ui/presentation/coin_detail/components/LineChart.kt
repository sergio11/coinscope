package com.dreamsoftware.coinscope.ui.presentation.coin_detail.components
...
import com.dreamsoftware.coinscope.models.ValueLabel
...
@Composable
fun LineChart(
    dataPointVOS: List<DataPointVO>,
    style: ChartStyle,
    visibleDataPointsIndices: IntRange,
    unit: String,
    modifier: Modifier = Modifier,
    selectedDataPointVO: DataPointVO? = null,
    onSelectedDataPoint: (DataPointVO) -> Unit = {},
    ...
    val visibleDataPoints = remember(dataPointVOS, visibleDataPointsIndices) {
        dataPointVOS.slice(visibleDataPointsIndices)
    }
    ...
    val selectedDataPointIndex = remember(selectedDataPointVO) {
        dataPointVOS.indexOf(selectedDataPointVO)
    }
    var drawPoints by remember {
        mutableStateOf(listOf<DataPointVO>())
    }
    var isShowingDataPoints by remember {
        mutableStateOf(selectedDataPointVO != null)
    }
    ...
    Canvas(
            ...
            .pointerInput(drawPoints, xLabelWidth) {
                detectHorizontalDragGestures { change, _ ->
                    ...
                    if(isShowingDataPoints) {
                        onSelectedDataPoint(dataPointVOS[newSelectedDataPointIndex])
                    }
                }
            }
    ) {
        ...
        drawPoints = visibleDataPointsIndices.map {
            ...
            val ratio = (dataPointVOS[it].y - minYValue) / (maxYValue - minYValue)
            val y = viewPortBottomY - (ratio * viewPortHeightPx)
            DataPointVO(
                x = x,
                y = y,
                xLabel = dataPointVOS[it].xLabel
            )
        }

        val conPoints1 = mutableListOf<DataPointVO>()
        val conPoints2 = mutableListOf<DataPointVO>()
        for(i in 1 until drawPoints.size) {
            ...
            conPoints1.add(DataPointVO(x, y1, ""))
            conPoints2.add(DataPointVO(x, y2, ""))
        }
...
private fun LineChartPreview() {
    CryptoTrackerTheme {
        ...
        LineChart(
            ...
            selectedDataPointVO = dataPointVOS[1]
        )
    }
}